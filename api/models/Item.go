...
type Item struct {
	...
	Name     float32    `gorm:"size:255;not null;unique" json:"name"`
...
func (i *Item) UpdateAnItem(db *gorm.DB) (*Item, error) {
...
func (i *Item) DeleteAnItem(db *gorm.DB, pid uint64) (*Item, error) {
	...
	err = db.Debug().Model(&Item{}).Where("id = ?", pid).Updates(Item{DeletedAt: time.Now()}).Error
	if err != nil {
		return &Item{}, err
	}
	return i, nil
